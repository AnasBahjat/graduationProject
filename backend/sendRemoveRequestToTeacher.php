<?php
    ...
    
    {
        ...
        if($isRequestExists == 1){
            ...
            $notificationBody = "$parentName Wants to delete an existing course , click to show more ..";

            $isNotificationSent = isNotificationSentBefore($teacherEmail,$notificationType,$notificationTitle,$notificationBody,$courseId,$conn);
            if($isNotificationSent == true){
                echo "Exists";
            }
            else {
                $setParentNotificationStmt = $conn->prepare("INSERT INTO notifications (ownerEmail,notificationType,notificationTitle,notificationBody,parentRequestId) VALUES(?,?,?,?,?)");
            $setParentNotificationStmt->bind_param("sissi",$teacherEmail,$notificationType,$notificationTitle,$notificationBody,$courseId);
    ...
    function isNotificationSentBefore($parentEmail,$notificationType,$notificationTitle,$notificationBody,$courseId,$conn){
        $SQL = $conn->prepare("SELECT notificationId FROM notifications WHERE ownerEmail = ? AND notificationType = ?
         AND notificationTitle = ? AND notificationBody = ? and parentRequestId = ? ;");
